...
function initBookingModal() {
  ...
  function openModal(sourceName) {
    ...
    // If form was already submitted, keep success state or reset if needed
    const firstInput = modal.querySelector('input[name="fullName"]');
    if (firstInput && formState.style.display !== 'none') {
  ...
  if (bookingForm) {
    bookingForm.addEventListener('submit', (e) => {
      ...
      setTimeout(() => {
        // Hide Form State, Show Success State
        if (formState && successState) {
          formState.style.display = 'none';
          successState.style.display = 'block';

          // Personalize success banner
          const userNameSpan = document.getElementById('confirmedUserName');
          if (userNameSpan) {
            userNameSpan.textContent = name.split(' ')[0] || 'Friend';
          }
        }
